# Devops-Capstone-Project

### Description

My Final Project for Devops cource created by IBM.This is a software project made from scratch and in it I have applied all the cources I have learned; this includes:
  - Creating sprint plans
  - Devloping the services using Test Driven Approach
  - Adding Continous Integration (CI) and Security
  - Deploying my application to Kubernetes
  - Building an automated CD Devops Pipeline

### Author

[John Rofrano](https://www.coursera.org/instructor/johnrofrano), Senior Technical Staff Member, DevOps Champion, @ IBM Research, and Instructor @ Coursera

### License

Licensed under the Apache License. See [LICENSE](LICENSE)

### <h3 align="center"> © IBM Corporation 2022. All rights reserved. <h3/>
